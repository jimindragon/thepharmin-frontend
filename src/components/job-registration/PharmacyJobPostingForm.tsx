...
import { analyzeDemo, PHARMACY_AI_FILL_DEMO, type PharmacyAiFillPatch } from "@/components/job-registration/aiFillDemo";
...
export function PharmacyJobPostingForm() {
  ...
  async function handleAiAnalyze(text: string) {
    const patch = await analyzeDemo(PHARMACY_AI_FILL_DEMO, text);
   ...
   */
  function applyAiDraft(patch: PharmacyAiFillPatch) {
    let applied = 0;
    ...
    apply(patch.salaryNote, setSalaryNote);
    apply(patch.mainPrescribingHospital, setMainPrescribingHospital);
              ...
              <div className="mb-4">
                <InfoNoticeBox>
                  ...
                  <span className="block text-[#6b7280]">
                    별도 연락처나 이메일 이동 없이, 지원자가 더파마 프로필과 이력서로 바로 지원할 수 있습니다. 접수 내역은 기업 센터에서 확인할 수 있습니다.
                  </span>
                </InfoNoticeBox>
              </div>
