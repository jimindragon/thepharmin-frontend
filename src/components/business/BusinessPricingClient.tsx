...
export function BusinessPricingClient() {
  ...
  const secondaryCta = isMember ? { href: "/business/support/inquiries", label: "고객센터에 문의하기" } : { href: "/business/support", label: "고객센터에 문의하기" };

  return (
    <>
      <BusinessPublicHeader />
      <main className="bg-white">
        <section className="bg-[#050505] px-6 py-20 text-white max-[760px]:py-14">
          <div className="app-shell--default">
            <Eyebrow tone="dark">PRICING</Eyebrow>
            <h1 className={`mt-3 max-w-[680px] text-white ${typeScale.heroTitle}`}>채용의 시작, 공고 등록은 무료입니다.</h1>
            <p className="mt-5 max-w-[620px] text-[15px] font-normal leading-[1.75] tracking-[-0.01em] text-white/72">
              제약·바이오·병원·약국 전문 인재를 위한 채용공고를 비용 없이 등록하세요. 더 많은 인재에게 빠르게 도달해야 할 때만 부스트를 선택해 공고 노출을 높일 수 있습니다.
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              {heroHighlights.map((highlight) => (
                <span key={highlight} className="inline-flex h-9 items-center gap-2 border border-white/18 bg-white/5 px-3.5 text-[12px] font-medium text-white/85">
                  <span className="h-1.5 w-1.5 shrink-0 bg-[#00a896]" />
                  {highlight}
                </span>
              ))}
            </div>
        ...
        <section className="px-6 py-20 max-[760px]:py-14">
          <div className="app-shell--default">
            <Eyebrow>HOW IT WORKS</Eyebrow>
            <h2 className={`mt-3 text-[#17202c] ${typeScale.sectionTitle}`}>무료로 등록하고, 필요할 때 노출을 높이세요</h2>
            <p className="mt-3 max-w-[640px] text-[14px] font-normal leading-[1.75] tracking-[-0.01em] text-[#68717e]">
              공고 등록은 무료입니다. 채용을 보다 빠르게 진행하고 싶을 때만 부스트를 선택해 목록 상단 노출과 인재 알림 혜택을 이용할 수 있습니다.
            </p>

            <div className="mt-8 grid grid-cols-2 gap-5 max-[900px]:grid-cols-1">
              <div className="border border-[#dfe4ea] bg-white p-7">
                <span className="inline-flex h-6 items-center border border-[#cfd8e3] bg-[#f7f8fa] px-2.5 text-[11px] font-medium text-[#596373]">기본 제공</span>
                <h3 className={`mt-4 text-[#17202c] ${typeScale.cardTitle}`}>무료 공고 등록</h3>
                <p className="mt-2 text-[13px] font-normal leading-[1.7] tracking-[-0.01em] text-[#68717e]">모든 채용 트랙에서 공고를 무료로 등록할 수 있습니다.</p>
                <div className="mt-5 divide-y divide-[#edf1f5] border-y border-[#edf1f5]">
                  {freePostingRows.map((row) => (
                    <div key={row.label} className="flex items-center justify-between gap-3 py-3 text-[13px]">
                      <span className="font-medium text-[#303946]">{row.label}</span>
                      <span className="font-normal text-[#68717e]">{row.value}</span>
                    </div>
                  ))}
                </div>
                <p className="mt-5 border border-[#e3e7ed] bg-[#fbfcfd] p-3 text-[12px] font-normal leading-[1.65] text-[#596373]">
                  서비스 오픈일로부터 1년 이내 입점한 기업은 프로모션 기간 동안 산업·연구·병원 트랙도 공고 수 제한 없이 무료로 게시할 수 있습니다.
                </p>
              </div>

              <div className="border border-[#dfe4ea] bg-white p-7">
                <span className="inline-flex h-6 items-center border border-[#cfd8e3] bg-[#f7f8fa] px-2.5 text-[11px] font-medium text-[#596373]">추가 노출 상품</span>
                <h3 className={`mt-4 text-[#17202c] ${typeScale.cardTitle}`}>부스트</h3>
                <p className="mt-2 text-[12px] font-medium text-[#8a94a3]">① 적용할 공고 선택 → ② 이용 기간 선택</p>
                  ...
                  {boostTiers.map((tier) => (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block text-[13px] font-medium text-[#303946]">{tier.label}</span>
                        <span className="block text-[12px] font-normal text-[#8a94a3]">{tier.description}</span>
                      </span>
                      <span className="shrink-0 text-right">
                        {tier.discount ? <span className="mr-2 inline-flex h-6 items-center bg-[#e8f0fc] px-2 text-[11px] font-medium text-[#337ddf]">{tier.discount}</span> : null}
                        {tier.originalPrice ? <span className="mr-1.5 text-[12px] font-normal text-[#b6bec9] line-through">{tier.originalPrice}</span> : null}
                        <span className="text-[14px] font-bold text-[#17202c]">{tier.price}</span>
                      </span>
                    </label>
                  ))}
                </div>
                ...
                <div className="mt-5 space-y-2 border-t border-[#edf1f5] pt-4">
                  {boostChecklist.map((item) => (
                    <div key={item} className="flex items-center gap-2 text-[12px] font-normal text-[#4f5967]">
        ...
        <section className="bg-[#050505] px-6 py-16 text-white">
          <div className="app-shell--default flex items-center justify-between gap-8 max-[760px]:flex-col max-[760px]:items-start max-[760px]:gap-5">
            <div>
              ...
              <h2 className="mt-3 max-w-[520px] text-[22px] font-bold leading-[1.4] tracking-[-0.02em] text-white">지금 입점하면 부스트를 최대 40% 할인받을 수 있습니다</h2>
        ...
        <section className="px-6 py-20 max-[760px]:py-14">
          <div className="app-shell--default">
            ...
            <h2 className={`mt-3 text-[#17202c] ${typeScale.sectionTitle}`}>자주 묻는 질문</h2>
            <div className="mt-8 grid grid-cols-2 gap-x-10 gap-y-7 max-[760px]:grid-cols-1">
              {faqItems.map((item) => (
                <div key={item.question} className="border-t border-[#dfe4ea] pt-4">
        ...
        <section className="bg-[#050505] px-6 py-20 text-center text-white max-[760px]:py-16">
          <div className="app-shell--default">
