...
import {
  ...
  researchAreaOptions,
  researchEmployeeCountOptions,
  researchOrgManager,
  researchOrgVerification,
  initialResearchOrgProfile,
  type ResearchProfileMaster,
  type ResearchInstitutionType,
  type ResearchEmployeeCountRange,
  type ResearchFileStatus,
} from "@/data/researchOrgProfile";

function statusLabel(status: ResearchFileStatus) {
...
function LogoMark({ profile }: { profile: ResearchProfileMaster }) {
...
export function ResearchOrgProfileClient() {
  const [profile, setProfile] = useState<ResearchProfileMaster>(initialResearchOrgProfile);
  const [newProduct, setNewProduct] = useState("");
  const [saved, setSaved] = useState(false);

  const completionItems = [
    { label: "기관 정보", done: true },
    ...
    { label: "담당자 정보", done: Boolean(researchOrgManager.email && researchOrgManager.phone) },
    { label: "계정 정보", done: false },
  ];
  const accountVerificationItems = [
    { label: "이메일 인증", detail: researchOrgManager.email, done: true },
    { label: "담당자 인증", detail: `${researchOrgManager.managerName} · ${researchOrgManager.phone}`, done: true },
    { label: "사업자 인증", detail: "기관 인증 완료", done: researchOrgVerification.verificationStatus === "approved" },
  ];
  const sectionStatus = (label: string) => completionItems.find((i) => i.label === label)?.done ? "완료" : "작성 중";

  const updateProfile = <K extends keyof ResearchProfileMaster>(key: K, value: ResearchProfileMaster[K]) => {
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between gap-5 max-[760px]:flex-col">
        <div>
          <PageBreadcrumb items={[{ label: "기업센터", href: "/business/dashboard" }, { label: "기관관리" }, { label: "기관정보 관리" }]} />
          <h1 className="mt-5 text-[34px] font-bold tracking-[-0.02em] text-[#17202c]">기관 정보 관리</h1>
          <p className="mt-2 text-[13px] font-normal text-[#68717e]">채용공고와 기관 상세 페이지에 표시되는 기관 정보를 관리합니다.</p>
        </div>
      ...
      <div className="mt-8 space-y-5">
        <SectionCard id="verification" title="인증 정보" description="인증 정보는 승인 상태와 인증 배지에 사용되며, 변경 시 운영팀 검토 후 반영됩니다." status="완료">
          ...
          <div className="flex items-center justify-between gap-3 border border-t-0 border-[#dfe4ea] px-4 py-4">
            <div>
              ...
              <p className="mt-2 text-[13px] font-medium text-[#17202c]">{statusLabel(researchOrgVerification.businessLicenseFile.status)}</p>
            </div>
        ...
        <SectionCard id="basic" title="기관 정보" description="기관의 공식 정보와 연락처를 관리합니다." status={sectionStatus("기관 정보")}>
          <div>
            <h3 className="text-[16px] font-bold tracking-[-0.02em] text-[#303946]">기관 유형</h3>
            <div className="mt-4 space-y-2">
              <FieldLabel required>기관 유형</FieldLabel>
              <Segmented value={profile.institutionType} options={institutionTypeOptions} onChange={(value: ResearchInstitutionType) => updateProfile("institutionType", value)} />
              <p className="text-[11.5px] font-normal leading-[1.55] text-[#8a94a3]">가입 시 선택한 기관 유형과 동일한 기준이며, 공고 등록 시 산업 분류 기본값으로 활용됩니다.</p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-5 border-t border-[#f0f2f5] pt-6">
            ...
            <div className="grid grid-cols-2 gap-4 max-[640px]:grid-cols-1">
              <div className="space-y-2">
                ...
                <Segmented value={profile.employeeCount} options={researchEmployeeCountOptions} onChange={(value: ResearchEmployeeCountRange) => updateProfile("employeeCount", value)} />
              </div>
              <div className="space-y-2">
                <FieldLabel required>기관 전화번호</FieldLabel>
                <TextInput value={profile.phone} onChange={(value) => updateProfile("phone", value)} />
              </div>
            </div>
            <div className="space-y-2">
              <FieldLabel required>기관 이메일</FieldLabel>
              <TextInput value={profile.email} onChange={(value) => updateProfile("email", value)} />
            </div>
          </div>
        </SectionCard>

        <SectionCard id="profile" title="공개 프로필" description="구직자에게 공개되는 기관 소개 정보입니다." status={sectionStatus("공개 프로필")}>
          <div className="grid grid-cols-[230px_minmax(0,1fr)] gap-6 max-[820px]:grid-cols-1">
            <div>
              <FieldLabel required>기관 로고</FieldLabel>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <FieldLabel>기관 소개 본문</FieldLabel>
                <textarea
        ...
        <SectionCard id="business" title="사업·채용 정보" description="구직자 검색과 공고 추천에 활용되는 기관 정보입니다." status={sectionStatus("사업·채용 정보")}>
          <div className="space-y-6">
            <div>
              <FieldLabel>주요 사업 영역</FieldLabel>
              <div className="mt-3 flex flex-wrap gap-2">
                {researchAreaOptions.map((item) => (
        ...
        <SectionCard id="manager" title="담당자 정보" description="채용 관련 문의와 서비스 안내를 받을 담당자 정보입니다." status={sectionStatus("담당자 정보")}>
          <div className="grid grid-cols-2 gap-5 max-[820px]:grid-cols-1">
            {[
              ["담당자명", researchOrgManager.managerName],
              ["부서", researchOrgManager.department],
              ["직책", researchOrgManager.position],
              ["이메일", researchOrgManager.email],
              ["연락처", researchOrgManager.phone],
              ["계정 ID", researchOrgManager.accountId],
            ].map(([label, value]) => (
