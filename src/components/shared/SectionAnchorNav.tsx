...
export interface AnchorSection {
   ...
   * 그 화면에 없는 목적지까지 한 행에서 잡아야 할 때 쓴다.
   *
   * **지금 href를 넘기는 호출부는 없다.** 기업 상세 개요가 잠시 이 형태로 면접 후기·기업 리뷰를 들었으나,
   * 그 둘을 본문에서 인라인으로 펼치는 쪽으로 방향이 잡히면서 전항목 앵커로 되돌렸다. 분기는 전부
   * no-op으로 떨어져 렌더가 달라지지 않으므로, 같은 자리가 다시 필요해질 때를 위해 남겨 둔다.
   *
 ...
 *
 * 라우트 항목(href)이 있다면 이 수에 포함해 센다 — 그쪽은 "훑을 것"이 아니라 "여기서만 갈 수 있는 곳"이라
 * 앵커가 적다고 함께 사라지면 안 된다. 지금은 href를 넘기는 호출부가 없어 앵커만 세는 것과 같다.
 */
...
export function SectionAnchorNav({ sections, ariaLabel = "섹션 바로가기" }: { sections: AnchorSection[]; ariaLabel?: string }) {
   ...
   */
  /** 폴백은 **첫 앵커 항목**이다 — resolved[0]을 그대로 쓰면 라우트 항목만 있는 화면에서 그쪽에
      밑선이 그어져 "지금 여기"를 잘못 말한다. 앵커가 없으면 활성도 없다.
      라우트 항목을 쓰는 호출부가 없는 지금은 resolved[0]과 결과가 같다. */
      ...
      <div className="flex h-11 items-stretch overflow-x-auto px-3">
        {resolved.map((section, index) => {
          ...
          /* 칸막이 — 앵커(누르면 스크롤)와 라우트(누르면 화면 교체)는 같은 행에 있어도 다른 일을 한다.
             ...
             라우트 항목이 없으면 경계 자체가 없어 선도 렌더되지 않는다. 지금은 href를 넘기는 호출부가
             없어 이 선을 그리는 화면도 없다(공고 상세 4트랙·기업 상세 개요 모두 0개). */
