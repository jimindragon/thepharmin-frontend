  ...
  return (
    <Link href={job.slug ? `/jobs/${job.slug}` : "/jobs"} className="group border border-[#dfe4ea] bg-white p-4 transition hover:border-[#111111]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[12px] font-black text-[#667181]">{job.company}</p>
          <h3 className="mt-2 line-clamp-2 text-[15px] font-black leading-[1.45] text-[#171d26] group-hover:underline">{job.title}</h3>
        </div>
        <span className="shrink-0 text-[13px] font-black text-danger">{job.deadlineLabel.replace("마감 ", "")}</span>
      </div>
      <p className="mt-4 text-[12px] font-bold text-[#7b8491]">
  ...
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_164px] gap-5 max-[900px]:grid-cols-1">
      ...
      <aside className="border border-[#dfe4ea] bg-[#fbfcfd] p-4">
        <p className="text-[13px] font-black text-[#171d26]">매출 규모</p>
        <div className="mt-4 space-y-3">
          {Object.entries(legendColors).map(([label, color]) => (
            <div key={label} className="flex items-center gap-2 text-[12px] font-bold text-[#5e6875]">
        ...
        <div className="mt-7 grid grid-cols-[minmax(0,1fr)_292px] gap-8 max-[1080px]:grid-cols-1">
          <article className="min-w-0">
            <header className="border-b border-[#dfe4ea] pb-8">
              <span className="inline-flex h-7 items-center border border-[#cfe2df] bg-[#f7fbfa] px-2.5 text-[11px] font-black text-[#0b756e]">
                {insight.category}
              </span>
              <h1 className="mt-5 text-[34px] font-black leading-[1.24] tracking-[0] text-[#111827] max-[720px]:text-[26px]">
                {insight.title}
              </h1>
              <p className="mt-4 text-[16px] font-semibold leading-[1.75] text-[#5f6876]">{insight.description}</p>
              <div className="mt-7 flex flex-wrap items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-3 text-[12px] font-bold text-[#7b8491]">
                  <span className="font-black text-[#303946]">THE PHARMA INSIGHT</span>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex h-10 items-center gap-2 border border-[#dfe4ea] bg-white px-3 text-[12px] font-black text-[#596373] hover:border-[#111111] hover:text-[#111111]"
                  >
                    <Share2 size={17} />
                    {copied ? "복사됨" : "공유"}
                  </button>
            ...
            <section className="mt-8 border border-[#dfe4ea] bg-[#f7f8fa] p-6">
              <h2 className="text-[16px] font-black text-[#111827]">핵심 요약</h2>
              <ul className="mt-4 space-y-2.5">
                {salaryInsightSummary.map((summary) => (
                  <li key={summary} className="flex gap-2 text-[13px] font-semibold leading-[1.75] text-[#4e5967]">
                    <span className="mt-2 h-1.5 w-1.5 shrink-0 bg-[#0b756e]" />
                    {summary}
                  </li>
                ))}
              </ul>
            ...
            <section className="mt-10">
              <h2 className="text-[22px] font-black tracking-[0] text-[#111827]">1. 평균 연봉 vs 평균 근속연수 매핑</h2>
              <p className="mt-3 text-[13px] font-semibold leading-[1.7] text-[#6f7783]">
            ...
            <section className="mt-10">
              <h2 className="text-[22px] font-black tracking-[0] text-[#111827]">2. 매출 규모별 분포</h2>
              <div className="mt-5 overflow-x-auto border border-[#dfe4ea] bg-white">
                <table className="w-full min-w-[620px] border-collapse text-left text-[13px]">
                  <thead className="bg-[#f5f6f7] text-[#3c4654]">
                    <tr>
                      <th className="border-r border-[#dfe4ea] px-4 py-3 font-black">구분</th>
                      <th className="border-r border-[#dfe4ea] px-4 py-3 text-right font-black">평균 연봉(만원)</th>
                      <th className="border-r border-[#dfe4ea] px-4 py-3 text-right font-black">평균 근속연수(년)</th>
                      <th className="px-4 py-3 text-right font-black">기업 수</th>
                    </tr>
                  </thead>
                  <tbody>
                    {salaryTenureTableRows.map((row) => (
                      <tr key={row.revenueSize} className={clsx(row.revenueSize === "전체 평균" && "bg-[#fbfcfd] font-black")}>
                        <td className="border-r border-t border-[#dfe4ea] px-4 py-3 font-bold text-[#3f4855]">{row.revenueSize}</td>
                        <td className="border-r border-t border-[#dfe4ea] px-4 py-3 text-right font-bold text-[#3f4855]">{row.averageSalary}</td>
                        <td className="border-r border-t border-[#dfe4ea] px-4 py-3 text-right font-bold text-[#3f4855]">{row.averageTenure}</td>
                        <td className="border-t border-[#dfe4ea] px-4 py-3 text-right font-bold text-[#3f4855]">{row.companyCount}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <p className="mt-3 text-[12px] font-semibold text-[#8a94a3]">출처: DART 사업보고서(2024), 각 사 공시 기준</p>
            </section>

            <section className="mt-12 border-t border-[#dfe4ea] pt-8">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <h2 className="text-[22px] font-black tracking-[0] text-[#111827]">데이터로 본 기업의 채용공고 확인하기</h2>
                  <p className="mt-2 text-[13px] font-semibold text-[#6f7783]">연봉·근속 데이터와 함께 볼 만한 제약바이오 채용공고입니다.</p>
                </div>
                <Link href="/jobs" className="text-[13px] font-black text-[#0b756e] hover:text-[#111111]">
                  전체 공고 보기
                </Link>
              </div>
          ...
          <aside className="self-start max-[1080px]:grid max-[1080px]:grid-cols-2 max-[720px]:grid-cols-1">
            <div className="sticky top-[92px] space-y-5 max-[1080px]:static">
              <section className="border border-[#dfe4ea] bg-white p-5">
                <h2 className="text-[16px] font-black text-[#111827]">연관 콘텐츠</h2>
                  ...
                  {relatedArticles.slice(0, 3).map((article) => (
                    <Link key={article.id} href={`/insights/${article.slug}`} className="group grid grid-cols-[1fr_72px] gap-3 border-b border-[#eef1f4] pb-4 last:border-b-0 last:pb-0">
                      <div className="min-w-0">
                        ...
                        <h3 className="mt-2 line-clamp-2 text-[13px] font-black leading-[1.45] text-[#252d39] group-hover:underline">{article.title}</h3>
                        <p className="mt-2 text-[11px] font-bold text-[#8a94a3]">{article.date}</p>
                      </div>
              ...
              <section className="border border-[#dfe4ea] bg-white p-5">
                <h2 className="text-[16px] font-black text-[#111827]">카테고리</h2>
                <div className="mt-3 divide-y divide-[#eef1f4]">
                  {insightCategories.slice(1).map((category) => (
                    <Link
                      ...
                      href="/insights"
                      className={clsx(
                        "flex h-10 items-center justify-between text-[13px] font-bold hover:text-[#111111]",
                        category === insight.category ? "text-[#0b756e]" : "text-[#4f5967]",
                      )}
                    >
                      {category}
              ...
              <section className="border border-[#dfe4ea] bg-[#fbfcfd] p-5">
                <h2 className="text-[16px] font-black text-[#111827]">추천 공고 연결</h2>
                <p className="mt-3 text-[13px] font-semibold leading-[1.7] text-[#68717e]">
                  데이터로 본 기업의 채용 현황과 실제 공고를 함께 비교해 보세요.
                </p>
                <Link href="/jobs" className="mt-5 inline-flex h-10 items-center gap-2 border border-[#111111] bg-[#111111] px-4 text-[12px] font-black text-white">
                  채용공고 보기
