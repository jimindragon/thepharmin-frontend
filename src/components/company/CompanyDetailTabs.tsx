...
export function CompanyDetailTabs({ companyId }: { companyId: string }) {
   ...
   *
   * 하위 탭(/jobs·/interviews·/reviews·/news)의 렌더는 그대로 두되, **≤760px에서는 그 넷이 개요의
   * 해당 앵커로 리다이렉트되므로(CompanyMobileOverviewRedirect) 이 행이 실제로 서는 것은 761px
   * 이상뿐이다.** 종전엔 그쪽에 앵커 행이 없어 여기까지 숨기면 오갈 수단이 사라진다는 이유로 남겼는데,
   * 이제는 그 폭에서 하위 탭 화면 자체에 머무르지 않는다. 아래 max-[760px] 분기와 마크업을 지우지 않는
