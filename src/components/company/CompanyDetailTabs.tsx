...
export function CompanyDetailTabs({ companyId }: { companyId: string }) {
   ...
   * 그 바 하나가 네 목적지를 모두 든다 — 공고·뉴스는 개요 본문의 미리보기 섹션을 가리키는 앵커로,
   * 면접 후기·기업 리뷰는 이 행과 같은 곳으로 가는 **라우트 항목**(href)으로 든다. 그 둘의
   * 미리보기 섹션은 이 폭에서 본문에 없다(CompanyReviewsPreviewSection의 max-[760px]:hidden).
   * 건수는 getCompanyDetailCounts로 이 행과 같은 출처를 쓴다.
   *
   * 하위 탭(/jobs·/interviews·/reviews·/news)에서는 그대로 둔다 — 그쪽에는 앵커 행이 없어
   * 여기까지 숨기면 좁은 화면에서 기업 상세 안을 오갈 수단이 사라진다.
   *
   * 개요에서 그 바가 렌더되지 않는 경우는 없다: 라우트 항목 2개는 섹션 유무와 무관하게 항상 있고
   * "정보" 카드는 조건 없이 렌더돼, 최악의 경우(소개·공고·뉴스가 모두 빠진 기관)에도 3항목으로
   * MIN_SECTIONS(3)를 채운다. 종전 근거였던 "면접 후기·기업 리뷰 미리보기가 0건이어도 빈 상태로
   * 남아 항상 4섹션 이상"은 그 두 섹션을 이 폭에서 걷어낸 뒤로는 성립하지 않는다.
   */
