      ...
      <div className="flex overflow-x-auto">
        {tabLabels.map((tab) => (
          <button
            ...
            className="h-14 shrink-0 border-r border-[#edf1f5] px-6 text-[13px] font-medium text-[#4f5967] transition hover:bg-[#f7f8fa] hover:text-[#111111] first:text-brand first:shadow-[inset_0_-2px_0_#111111]"
          >
            {tab.label}
          </button>
        ))}
  ...
  return (
    <article className="border border-[#e0e6ee] bg-white p-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[13px] font-medium text-[#2f3845]">
          {review.role} · {review.career}
        </p>
        <span className="text-[12px] font-medium text-[#98a2b0]">{review.date}</span>
      </div>
      <div className="mt-3 flex flex-wrap gap-1.5">
        {review.keywords.map((keyword) => (
          <span key={keyword} className="border border-[#e2e8ef] bg-[#f8fafb] px-2 py-1 text-[11px] font-medium text-[#5f6876]">
            {keyword}
          </span>
        ))}
      </div>
      <p className="mt-3 text-[13px] font-normal leading-[1.75] text-[#3f4855]">{review.summary}</p>
      <div className="mt-4 flex gap-2 text-[12px] font-medium text-[#7d8796]">
  ...
  return (
    ...
    >
      {profile.interviewReviews.length ? (
        ...
        <EmptyState message="아직 등록된 후기가 없습니다. 이 기업에 대한 경험을 공유해보세요." cta={<button className="h-10 border border-[#111111] px-4 text-[13px] font-medium">면접 후기 작성하기</button>} />
      )}
    </SectionShell>
  );
}
  ...
  return (
    ...
    >
      {profile.employeeReviews.length ? (
        <>
          <div className="grid grid-cols-2 gap-3 max-[720px]:grid-cols-1">
            <div className="border border-[#e2e8ef] bg-[#fbfcfd] p-4">
              <p className="text-[13px] font-medium text-[#202733]">긍정 키워드</p>
            ...
            <div className="border border-[#e2e8ef] bg-[#fbfcfd] p-4">
              <p className="text-[13px] font-medium text-[#202733]">개선 키워드</p>
              <div className="mt-3 flex flex-wrap gap-2">
        ...
        <EmptyState message="아직 등록된 후기가 없습니다. 이 기업에 대한 경험을 공유해보세요." cta={<button className="h-10 border border-[#111111] px-4 text-[13px] font-medium">기업 후기 작성하기</button>} />
      )}
    </SectionShell>
  );
}
  ...
  return (
    <aside className="sticky top-[88px] grid h-fit gap-4 self-start max-[1120px]:static">
      ...
      <section className="border border-border bg-white p-5 shadow-[var(--shadow)]">
        ...
        <p className="mt-2 text-[12px] font-normal leading-[1.65] text-[#667181]">기업 후기와 면접 후기를 더 자세히 확인하려면 후기를 작성하거나 열람권을 사용하세요.</p>
